...
export function TaskManager({ initialTasks }: TaskManagerProps) {
  ...
  const { data: tasks = initialTasks, isFetching: isSearching } = useQuery<Task[]>({
    ...
    // Only use initialData from server props if we aren't searching
    initialData: debouncedQuery.trim() === "" ? initialTasks : undefined,
  });
  ...
  const saveTaskMutation = useMutation({
    ...
    onMutate: async (taskData) => {
      ...
      queryClient.setQueryData<Task[]>(["tasks", debouncedQuery], (old) => {
        ...
        if (editingTask) {
          ...
          // Optimistically add with a temporary fake ID
    ...
    onSuccess: (savedTask, variables, context) => {
      // Replace the optimistic task with the real task from the server
      queryClient.setQueryData<Task[]>(["tasks", debouncedQuery], (old) => {
        if (!old) return [];
        if (editingTask) {
          return old.map(t => t.id === savedTask.id ? savedTask : t);
        } else {
          // Find the optimistic task (which doesn't match the real ID) and replace it
          // Or just replace the first item since it was prepended
          const [optimisticTask, ...rest] = old;
          return [savedTask, ...rest];
        }
      });
    },
  ...
  const deleteTaskMutation = useMutation({
    ...
    // Removed invalidateQueries to prevent secondary API call
    onSettled: (data, error) => {
      if (!error) {
            ...
            paginatedTasks.map((task) => (
              <div key={task.id} className="flex w-full px-6 py-4 items-center border-b border-zinc-200 dark:border-zinc-800/80 hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                ...
                <div className="w-[15%] pr-4">
                  <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ring-1 ring-inset ${task.status === "done" ? "bg-green-50 text-green-700 ring-green-600/20 dark:bg-green-500/10 dark:text-green-400 dark:ring-green-500/20" :
